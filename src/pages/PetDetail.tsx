...
const SkillItem: React.FC<{ skill: Skill; onClick: () => void }> = ({
  skill,
  onClick,
}) => (
  <button
...
export const PetDetail: React.FC<{
  ...
  const [activeTab, setActiveTab] = useState<"stats" | "inventory">(
    location.state?.openInventory ? "inventory" : initialTab,
  );
  ...
  const [invCategoryFilter, setInvCategoryFilter] = useState<
    "all" | "egg" | "artifact" | "skill"
  >("all");
  const [invStatFilter, setInvStatFilter] = useState<string | "all">("all");
  const [invSkillTypeFilter, setInvSkillTypeFilter] = useState<string | "all">(
    "all",
  );
  const [invAffinityFilter, setInvAffinityFilter] = useState<string | "all">(
    "all",
  );
  ...
  const costPerLevel = pet
    ? Math.floor(pet.level * 500 * Math.pow(1.05, pet.level - 1))
    : 0;
  const expNeeded = pet ? getExpNeeded(pet.level) : 0;
  const canLevelUp = pet
    ? pet.experience >= expNeeded && pet.level < 100
    : false;
  ...
  const rankIndex = [
    ...
    "UX",
    "Z",
  ].indexOf(potentialRankCode || "F");
  const expectedSkills = rankIndex * 2 + 2;
  const isMajorEvolution =
    pet &&
    potentialRank &&
    (potentialRankCode !== currentRankCode ||
      (pet.level >= 11 && (pet.skills || []).length < expectedSkills));
  ...
  const allocatePoint = (stat: keyof typeof pet.stats) => {
    ...
    setProgress((prev) => ({
      ...prev,
      pets: (Array.isArray(prev.pets) ? prev.pets : []).map((p) =>
        p.id === pet.id
          ? {
              ...p,
  ...
  const handleSellSkill = (skillCode: string) => {
    const skill = pet.skills?.find(
      (s) => s.code === skillCode || s.id === skillCode,
    );
    if (!skill) return;
        ...
        p.id === pet.id
          ? {
              ...
              skills: (p.skills || []).filter(
                (s) => (s.code || s.id) !== (skill.code || skill.id),
              ),
            }
          : p,
      ),
    ...
    <div className="space-y-8 pb-10">
            ...
            {activeTab === "stats" ? (
              ...
              >
                <div className="flex flex-col gap-4 max-w-sm">
                  <div className="flex gap-4 w-full">
                    <NeonButton
                      onClick={() => {
                        ...
                        navigate(
                          `/battle/${pet.id}/${Math.random().toString(36).substring(7)}`,
                        );
                      }}
                  ...
                  <div className="flex gap-4 w-full">
                    <NeonButton
                      onClick={() => navigate(`/inventory/${pet.id}`)}
                      className="py-2 sm:py-3 text-[12px] sm:text-sm px-3 bg-sticker-pink flex-1"
                    >
                    ...
                    <NeonButton
                      onClick={() => {
                        if (canVisitEvolve) navigate(`/evolve/${pet.id}`);
                      }}
                      disabled={!canVisitEvolve}
                      className={cn(
                        "py-2 sm:py-3 text-[12px] sm:text-sm px-3 border-none flex-1",
                        !canVisitEvolve
                          ? "bg-transparent border-2 border-black/10 text-black/40 opacity-50"
                          : evolveBtnPulse
                            ? "bg-pen-red text-white animate-pulse"
                            : "bg-transparent border-2 border-pen-blue text-pen-blue",
                      )}
                    >
                      <Sparkles
                        className={cn(
                          "h-4 w-4 mr-1 sm:mr-2 shrink-0",
                          evolveBtnPulse
                            ? "text-white"
                            : !canVisitEvolve
                              ? "text-black/40"
                ...
                <div className="border-2 border-pen-blue p-4 sm:px-6 rounded-sm bg-white/50 ledger-grid">
                  <div className="flex items-center justify-between mb-4 border-b-2 border-pen-blue pb-2 px-1">
                    ...
                    {pet.statPoints > 0 && (
                      <div className="bg-pen-red text-white px-3 py-1 rounded-full text-[12px] font-black animate-pulse">
                  ...
                  <div className="grid grid-cols-1 gap-2 px-1">
                    {(
                      ...
                      <StatItem
                        ...
                        value={pet.stats?.[key] || 0}
                        passiveBonus={getPassiveBonus(pet, key)}
                        max={
                          key === "health"
                            ? Math.max(999, pet.stats?.maxHealth || 0)
                            : 999
                        }
                        showAdd={(pet?.statPoints || 0) > 0}
                ...
                >
                  ...
                  >
                    ...
                    <ClassificationItem
                      label="Класс"
                      value={pet.classification?.class}
                    />
                    <ClassificationItem
                      label="Отряд"
                      value={pet.classification?.order}
                    />
                    <ClassificationItem
                      label="Семейство"
                      value={pet.classification?.family}
                    />
                    <ClassificationItem
                      label="Род"
                      value={pet.classification?.genus}
                    />
                    <ClassificationItem
                      label="Вид"
                      value={pet.classification?.species}
                    />
                  </div>
                </div>

                <div className="w-full text-left mt-10 mb-4 px-1">
                  <h3 className="text-[20px] font-black text-[#0047ab] tracking-tight drop-shadow-none">
                    Навыки сущности
                  </h3>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 w-full px-1">
                  {(pet.skills || []).map((skill, i) => {
                    const colors: ("white" | "yellow" | "blue" | "pink")[] = [
                      "yellow",
                      "blue",
                      "pink",
                      "white",
                      ...
                      <GlassCard
                        ...
                        className={cn(
                          ...
                          cardColor === "yellow"
                            ...
                            : cardColor === "blue"
                              ? "bg-sticker-blue/80 hover:bg-sticker-blue"
                              : cardColor === "pink"
                                ? "bg-sticker-pink/80 hover:bg-sticker-pink"
              ...
              >
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        invCategoryFilter === cat.id
                          ? "bg-pen-blue text-white border-pen-blue"
                          : "bg-white text-pen-blue/40 border-pen-blue/10 hover:border-pen-blue/20",
                      )}
                    >
                      {cat.label}
                    </button>
                ...
                {/* Sub-filters */}
                {(invCategoryFilter === "artifact" ||
                  invCategoryFilter === "skill" ||
                  ...
                  <div className="space-y-2 p-3 bg-pen-blue/5 border border-pen-blue/10 rounded-lg text-left">
                    ...
                    {(invCategoryFilter === "skill" ||
                      ...
                      <div className="flex flex-wrap gap-2 text-[9px] font-black text-pen-blue/40 tracking-tighter">
                        ...
                        {["all", "passive", "active_buff", "active_debuff"].map(
                          (t) => (
                            <button
                              key={t}
                              onClick={() => setInvSkillTypeFilter(t)}
                              className={cn(
                                "px-2 py-0.5 rounded-full border",
                                invSkillTypeFilter === t
                                  ? "bg-pen-blue text-white"
                                  : "bg-white",
                              )}
                            >
                    ...
                    {(invCategoryFilter === "skill" ||
                      ...
                      <div className="flex flex-wrap gap-2 text-[9px] font-black text-pen-blue/40 tracking-tighter">
                        ...
                        {[
                          ...
                          <button
                            key={a.id}
                            onClick={() => setInvAffinityFilter(a.id)}
                            className={cn(
                              "px-2 py-0.5 rounded-full border",
                              invAffinityFilter === a.id
                                ? "bg-pen-blue text-white"
                                : "bg-white",
                            )}
                          >
                ...
                <div className="grid grid-cols-3 gap-3">
                  {(() => {
                    const filtered = (progress.inventory || []).filter(
                      (item) => {
                        const categoryMatch =
                          invCategoryFilter === "all" ||
                          item.type === invCategoryFilter;
                        if (!categoryMatch) return false;

                        const stat =
                          item.type === "artifact"
                            ? item.effect?.stat
                            : item.skillData?.targetStat;
                        const sMatch =
                          invStatFilter === "all" || stat === invStatFilter;
                        const typeMatch =
                          invSkillTypeFilter === "all" ||
                          (item.type === "skill" &&
                            item.skillData?.type === invSkillTypeFilter);

                        const affinityMatch =
                          invAffinityFilter === "all" ||
                          (item.type === "skill" &&
                            (item.skillData?.element === invAffinityFilter ||
                              item.skillData?.attribute === invAffinityFilter));

                        if (invCategoryFilter === "egg") return true;
                        if (invCategoryFilter === "artifact") return sMatch;
                        if (invCategoryFilter === "skill")
                          return sMatch && typeMatch && affinityMatch;
                    ...
                    if (filtered.length === 0)
                      return (
                        <div className="col-span-full py-12 text-pen-blue/20 font-black">
                          Пусто
                        </div>
                      );
                          ...
                          <div className="flex flex-col items-center justify-start h-full w-full p-2 relative overflow-visible">
                            <div className="flex-1 flex items-center justify-center w-full overflow-hidden">
                              <ItemIcon
                                ...
                                fallbackEmoji={item.fallbackEmoji}
                                className={cn(
                                  "group-hover:scale-110 transition-transform max-h-full",
                                  item.type === "egg"
                                    ? "w-20 h-20"
                                    : "text-6xl",
                                )}
                              ...
                              {item.skillData ? (
                                <div className="flex flex-col items-center gap-0.5 mt-1">
                                  <div className="text-[12px] font-black bg-white/80 border border-[#0047ab]/10 relative z-20 whitespace-nowrap text-[#0047ab] rounded-full px-1.5 py-0.5 tracking-tighter">
                                    {item.skillData.value}% •{" "}
                                    {item.skillData.type === "passive"
                                      ? "Пассив"
                                      : item.skillData.type === "active_buff"
                                        ? "Бафф"
                                        : "Дебафф"}
                                  </div>
                                  <div className="text-[12px] font-black text-[#0047ab] italic">
                                    {item.skillData.type === "passive"
                                      ? item.skillData.attribute === "light"
                                        ? "Свет"
                                        : item.skillData.attribute === "dark"
                                          ? "Тьма"
                                          : item.skillData.attribute === "time"
                                            ? "Время"
                                            : "Пустота"
                                      : item.skillData.element === "fire"
                                        ? "Огонь"
                                        : item.skillData.element === "water"
                                          ? "Вода"
                                          : item.skillData.element === "air"
                                            ? "Воздух"
                                            : "Земля"}
                                  </div>
                                </div>
      ...
      <InfoModal
        ...
        title={
          modalType?.rank
            ? "Ранг Сущности"
            : modalType?.stats
              ? "Аналитика Потенциала"
              : modalType?.element
                ? "Элемент"
                : modalType?.attribute
                  ? "Атрибут"
                  : modalType?.selectedSkill
                    ? modalType.selectedSkill.name
                    : "Детали"
        }
        showClose={true}
      ...
      >
        {modalType?.fullScreenImage ? (
          ...
          <div className="space-y-6">
            ...
            >
                ...
                <div className="space-y-1">
                  ...
                  <div className="text-[16px] font-black text-[#0047ab] capitalize">
                    {modalType.selectedSkill.type === "passive"
                      ...
                      : "Стихия: "}
                    {modalType.selectedSkill.type === "passive"
                      ? modalType.selectedSkill.attribute === "light"
                        ? "Свет"
                        : modalType.selectedSkill.attribute === "dark"
                          ? "Тьма"
                          : modalType.selectedSkill.attribute === "time"
                            ? "Время"
                            : "Пустота"
                      : modalType.selectedSkill.element === "fire"
                        ? "Огонь"
                        : modalType.selectedSkill.element === "water"
                          ? "Вода"
                          : modalType.selectedSkill.element === "air"
                            ? "Воздух"
                            : "Земля"}
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4 border-t border-[#0047ab]/10 pt-4 pb-6">
                <div>
                  <div className="text-[16px] font-black text-[#0047ab] tracking-tighter">
                    Влияние
                  </div>
                  <div className="text-[16px] font-black text-[#0047ab]">
                    {modalType.selectedSkill.value}%
                  </div>
                </div>
                <div>
                  <div className="text-[16px] font-black text-[#0047ab] tracking-tighter">
                  ...
                  <div className="text-[16px] font-black text-[#0047ab] capitalize">
                    {modalType.selectedSkill.targetStat === "health"
                      ? "Здоровье"
                      : modalType.selectedSkill.targetStat === "attack"
                        ? "Атака"
                        : modalType.selectedSkill.targetStat === "defense"
                          ? "Защита"
                          : modalType.selectedSkill.targetStat === "speed"
                            ? "Скорость"
                            : modalType.selectedSkill.targetStat === "magic"
                              ? "Магия"
                              : "Регенерация"}
                  </div>
                </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-2">
              {[
                ...
                const isSelected =
                  pet.level >= min && pet.level <= (max || min);
                return (
                  <div
                    key={i}
                    className={cn(
                  ...
                  >
                    <span className="text-[16px] font-black text-pen-blue">
                      {r.range}
                    </span>
                    <span className="text-[16px] font-black text-pen-blue">
                      {r.code} - {r.label}
          ...
          <div className="space-y-4">
            <p className="text-[16px] font-black text-pen-blue leading-relaxed border-b border-black/5 pb-4">
              Начальные характеристики и потенциал роста зависят от редкости
              питомца:
            </p>
            <div className="grid grid-cols-1 gap-2">
              {[
          ...
          <TypeChartContent
            element={modalType?.element}
            attribute={modalType?.attribute}
          />
        )}
      </InfoModal>
        ...
        {itemToUse && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1, y: 0 }}
              className="w-full max-w-sm bg-white border-4 border-pen-blue p-6 shadow-2xl relative"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="text-center mb-6">
                <h2 className="text-2xl font-black text-pen-blue mb-2">
                  Применить
                </h2>
                <p className="text-sm text-pen-blue/60 font-bold">
                  Выберите питомца
                </p>
              </div>
              <div className="space-y-3 max-h-[40vh] overflow-y-auto pr-2 force-scrollbar">
                {(() => {
                  const eligiblePets = (progress.pets || []).filter((p) => {
                    if (itemToUse.type !== "skill" || !itemToUse.skillData)
                      return true;

                    // Check if pet already has this skill by name
                    const hasSkill = (p.skills || []).some(
                      (s) => s.name === itemToUse.name,
                    );
                  ...
                  if (eligiblePets.length === 0) {
                    const reqRu =
                      itemToUse.skillData?.type === "passive"
                        ? itemToUse.skillData.attribute === "light"
                          ? "Свет"
                          : itemToUse.skillData.attribute === "dark"
                            ? "Тьма"
                            : itemToUse.skillData.attribute === "time"
                              ? "Время"
                              : "Пустота"
                        : itemToUse.skillData.element === "fire"
                          ? "Огонь"
                          : itemToUse.skillData.element === "water"
                            ? "Вода"
                            : itemToUse.skillData.element === "air"
                              ? "Воздух"
                              : "Земля";
                    ...
                    return (
                      <div className="py-8 px-4 text-center border-2 border-dashed border-pen-blue/20 rounded-lg">
                        <div className="text-sm font-black text-pen-blue/60 italic leading-relaxed">
                          У вас нет питомцев с подходящим{" "}
                          {itemToUse.skillData?.type === "passive"
                            ? "атрибутом"
                            : "стихией"}{" "}
                          ({reqRu}) для этого навыка.
                        </div>
                      </div>
                    );
                  }
                  ...
                  return eligiblePets.map((p) => (
                    <button
                      key={p.id}
                      onClick={() => handleUseItem(itemToUse, p.id)}
                      className="w-full flex items-center gap-3 p-3 bg-pen-blue/5 hover:bg-pen-blue/10 border-2 border-transparent hover:border-pen-blue transition-all group"
                    >
                      <img
                        src={p.image}
                        className="h-12 w-12 rounded-lg border border-pen-blue/20 overflow-hidden shrink-0 bg-white object-cover"
                      />
                      <div className="text-left flex-1 min-w-0 font-black text-pen-blue truncate">
                        {p.name}
                      </div>
                    </button>
                  ));
              ...
              <NeonButton
                onClick={() => setItemToUse(null)}
                className="w-full mt-6 py-2 bg-pen-blue/5 border-2 border-black/10 text-pen-blue text-sm font-black italic"
              >
                Отмена
              </NeonButton>
            </motion.div>
          ...
          >
            ...
            >
              <div className="p-8 text-center space-y-6">
                ...
                <div className="space-y-1">
                  <div className="text-[12px] font-black text-pen-blue/40 tracking-[0.2em]">
                    Информация
                  </div>
                  <h3 className="text-2xl font-black text-pen-blue leading-tight">
                    {selectedItemInfo.name}
                  </h3>
                  <div className="text-[10px] font-black text-pen-blue/30 tracking-widest">
                    {selectedItemInfo.type === "artifact"
                      ? "артефакт"
                      : selectedItemInfo.type === "skill"
                        ? "навык"
                        : "яйцо"}
                  </div>
                </div>
                <p className="text-[16px] font-bold text-pen-blue/60 leading-relaxed italic">
                  {selectedItemInfo.description}
                </p>

                {selectedItemInfo.type === "artifact" &&
                  selectedItemInfo.effect && (
                    <div className="p-3 bg-pen-blue/5 border border-pen-blue/20 rounded flex items-center justify-center gap-2">
                      ...
                      <span className="text-[20px] font-black text-pen-blue">
                        +{selectedItemInfo.effect.value}{" "}
                        {STAT_MAP_RU[selectedItemInfo.effect.stat] ||
                          selectedItemInfo.effect.stat}
                      </span>
                    </div>
                  )}
                  ...
                  selectedItemInfo.skillData && (
                    <div className="p-3 bg-pen-blue/5 border border-pen-blue/20 rounded flex flex-col items-center gap-1">
                      <div className="flex flex-col items-center gap-1 mb-2">
                        <div className="text-[10px] font-black bg-white/80 border border-[#0047ab]/10 relative z-20 whitespace-nowrap text-pen-blue rounded-full px-2 py-0.5 tracking-tighter">
                          {selectedItemInfo.skillData.type === "passive"
                            ? "Пассивный"
                            : selectedItemInfo.skillData.type === "active_buff"
                              ? "Боевой Бафф"
                              : "Боевой Дебафф"}
                        </div>
                        <div className="text-[12px] font-black text-pen-blue/70">
                          {selectedItemInfo.skillData.type === "passive"
                            ...
                            : "Стихия: "}
                          {selectedItemInfo.skillData.type === "passive"
                            ? selectedItemInfo.skillData.attribute === "light"
                              ? "Свет"
                              : selectedItemInfo.skillData.attribute === "dark"
                                ? "Тьма"
                                : selectedItemInfo.skillData.attribute ===
                                    "time"
                                  ? "Время"
                                  : "Пустота"
                            : selectedItemInfo.skillData.element === "fire"
                              ? "Огонь"
                              : selectedItemInfo.skillData.element === "water"
                                ? "Вода"
                                : selectedItemInfo.skillData.element === "air"
                                  ? "Воздух"
                                  : "Земля"}
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <Sparkles className="h-6 w-6 text-pen-blue/40" />
                        <span className="text-[20px] font-black text-pen-blue">
                          +{selectedItemInfo.skillData.value}%{" "}
                          {STAT_MAP_RU[selectedItemInfo.skillData.targetStat] ||
                            selectedItemInfo.skillData.targetStat}
                        </span>
                      </div>
                    </div>
                  )}
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <button
                    onClick={() => setSelectedItemInfo(null)}
                    className="text-[16px] font-black text-pen-blue/40 tracking-widest py-2"
                  >
                    Отмена
                  </button>
                </div>
      ...
      >
        <div className="space-y-4">
          <p className="text-[16px] font-black text-pen-red leading-relaxed border-b border-black/5 pb-4">
            Внимание! Ваш текущий ранг призывателя{" "}
            <strong>"{limitError?.name}"</strong>.
          </p>
          <div className="text-sm font-bold text-pen-blue/80">
            Он позволяет иметь не более{" "}
            <span className="text-xl text-pen-blue">{limitError?.limit}</span>{" "}
            питомцев.
          </div>
          <div className="bg-white/50 p-4 border border-black/10 text-xs font-black text-pen-blue italic rounded-sm">
            Чтобы повысить ранг и призывать больше сущностей, вам необходимо
            развивать и прокачивать своих текущих питомцев!
          </div>
          <NeonButton onClick={() => setLimitError(null)} className="w-full">
            Понятно
          </NeonButton>
        </div>
      </InfoModal>
  ...
  >
    ...
    >
      <motion.div
        initial={{ width: 0 }}
        animate={{
          width: `${Math.min((((value || 0) + (passiveBonus || 0)) / max) * 100, 100)}%`,
        }}
...
const ClassificationItem = ({
  ...
  <div className="space-y-0.5 min-w-0">
    <div
      className="font-black leading-tight text-[11px] sm:text-[12px] opacity-70"
      style={{ color: "#0047ab" }}
    >
      {label}
    </div>

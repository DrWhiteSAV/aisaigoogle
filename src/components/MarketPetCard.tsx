...
export const MarketPetCard: React.FC<MarketPetCardProps> = ({ 
  pet, 
  showPrice, 
  className = "",
  onClick,
  ...
  // As requested: font size 12px everywhere inside
  const fontSize = "text-[12px]";
  const nameFontSize = "text-[12px]";
      ...
      <div className="w-full h-full relative group shadow-sm bg-white border-2 border-black/5 overflow-hidden">
        ...
        <div className={cn(
          ...
          <div className="flex justify-between items-start gap-1 mb-0.5">
            <h3 className={cn("font-black text-pen-blue leading-[1.1] text-balance", fontSize)}>
              {pet.name}
            </h3>
          ...
          {showPrice !== undefined && (
            <div className={cn("font-black text-pen-blue mt-1.5 italic text-center w-full", fontSize)}>{showPrice} 🌱</div>
          )}
        </div>
      </div>
    </motion.div>
